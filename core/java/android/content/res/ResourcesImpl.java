                ...
                if (id != 0) {
                    try {
                        ...
                        if (AccentUtils.isResourceFooterTextColor(resName))
                            value.data = AccentUtils.getNewFooterTextColorColor(value.data);
    ...
    ComplexColor loadComplexColor(Resources wrapper, @NonNull TypedValue value, int id,
        ...
        if (id != 0) {
            try {
                ...
                if (AccentUtils.isResourceAccent(resName))
                    value.data = AccentUtils.getNewAccentColor(value.data);
                if (AccentUtils.isResourceFooterText(resName))
                    value.data = AccentUtils.getNewFooterTextColor(value.data);
            } catch (NotFoundException ignored) {
            } catch (Exception ex) {
                Log.e(TAG, ex.getMessage());
            }
